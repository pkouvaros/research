...
\begin{tikzpicture}[node distance=3cm, font=\scriptsize,auto,
  ...
  \node [label=left:{$[0,1]$},fill=green!50!white,draw=green,neuron] (1)
  {$\out{0,1}$};

  \node [label=left:{$[-1,1]$},below of=1,yshift=1.5cm,
  fill=green!50!white,draw=green,neuron] (2) {$\out{0,2}$};

  \node [label=above:{$[-1,2]$},right of=1, xshift=-1.5cm,
  fill=gray!50!white,draw=gray,neuron] (3) {$\pre{1,1}$};

  \node [label=below:{$[-2,1]$},minimum size=1.5em, below of=3,yshift=1.5cm,
  fill=gray!50!white,draw=gray,neuron] (4) {$\hat{q}$};

  \node [label=above:{$[0,2]$},right of=3, xshift=-1.5cm,
  fill=gray!50!white,draw=gray,neuron] (3b) {$\out{1,1}$};

  \node [label=below:{$[0,1]$},minimum size=1.5em,below of=3b,yshift=1.5cm,
  fill=gray!50!white,draw=gray,neuron] (4b) {$q$};


  \node [label=above:{$[-1,2]$},right of=3b, minimum size=1.5em, xshift=-1.5cm,
  fill=gray!50!white,draw=gray,neuron] (5) {$r$};

  \node [label=below:{$[0,3]$},below of=5,yshift=1.5cm,
  ...
  (4b) edge[very thick,red] node [near start,left,color=black] {$-1$} (5)
